...
**Every bees are connected to the hive with a bi-directionnal Peer-to-Peer connection thanks to [WebRTC][webrtc].**
...
Making 2 clients communicating generally consists in broadcasting messages from the server to all clients 
(see the schema).
...
with who we was able to make that demo from scratch in one day!
